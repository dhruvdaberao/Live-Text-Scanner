...
//             {!cameraAvailable && <p className="text-yellow-400 text-center mt-4">Camera not found or supported on this device.</p>}
//           </main>
//           <footer className="w-full max-w-4xl mx-auto text-gray-400 text-left px-4 py-8 mt-4">
//             <div className="grid md:grid-cols-2 gap-8 text-sm">
//               <div>
//                 <h3 className="font-semibold text-lg text-white mb-2">How to Use</h3>
//                 <ol className="list-decimal list-inside space-y-1">
//                   <li>Press <strong>Start Camera</strong>.</li>
//                   <li>Point your camera at any text and tap <strong>Scan Frame</strong>.</li>
//                   <li>Scan multiple text blocks if your question is long.</li>
//                   <li>Tap <strong>Get Answer</strong> to let Gemini solve it!</li>
//                 </ol>
//               </div>
//               <div>
...
//                   <li>Solve homework problems by scanning questions directly.</li>
//                 </ul>
//               </div>
//             </div>
//           </footer>
...
const App: React.FC = () => {
  ...
  // Existing state for the scanner app
  const [isCameraActive, setIsCameraActive] = useState<boolean>(false);
  const [scannedTexts, setScannedTexts] = useState<string[]>([]);
  const [apiResponse, setApiResponse] = useState<string>('');
  const [isScanning, setIsScanning] = useState<boolean>(false);
  ...
  const handleClearApiKey = () => {
    ...
    setApiKey(null);
    initializeApi(''); // Clear the client instance
  };
  ...
  const handleApiError = (apiError: any) => {
    ...
    if (apiError instanceof Error) {
      if (apiError.message.includes('429') || apiError.message.includes('RESOURCE_EXHAUSTED')) {
        ...
        handleClearApiKey(); // Clear the bad key and force re-entry
      }
    }
    setError(errorMessage);
  };
      ...
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden">
        ...
        <div className="w-full max-w-md mx-auto z-10 text-center">
          ...
          <div className="text-xs text-gray-500 mt-6 text-left p-4 bg-gray-800/50 rounded-lg">
            ...
            <p>Your API key is saved only in your browser's local storage. It is not sent to any servers besides Google's.</p>
          </div>
        </div>
      </div>
    );
  ...
  // Render Camera View if key is set and camera is active
  const getAnswerDisabled = isGettingAnswer || scannedTexts.length === 0 || isAnswerCoolingDown;
